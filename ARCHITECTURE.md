...
(per call), with no shared session pool. The agent service is `localhost`
...
| `/ingest`      | POST   | (none)                                 | `{new, skipped, total, files}` (idempotent)  |
...
process, with no cross-process cache invalidation to worry about.
...
- **`rag_context`** is **pure retrieval**. It calls
